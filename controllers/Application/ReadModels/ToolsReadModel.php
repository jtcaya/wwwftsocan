...
class ToolsReadModel extends AggregateReadModel
{
    const READ_COMPLETED = 'tools_read_completed';
    ...
    public function __invoke(AggregateEvent $event)
    {
        if (is_null($this->commandRunner)) {
            ...
            $this->commandRunner = new CommandRunner($app, 'tools:read', $arguments, $swoole);
        }
        ...
        $event = new ReadToolsCompleted(
            $aggregateValueObject,
            $event->getAggregateRootName(),
            ToolsReadModel::READ_COMPLETED
        );
